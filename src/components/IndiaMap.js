...
export default function IndiaMap({ highlighted, setHighlighted }) {
    ...
    return (
      <div className="flex items-center justify-center h-48 sm:h-64 md:h-80 lg:h-96">
        <div className="text-center">
          ...
          <p className="text-gray-300 text-xs sm:text-sm md:text-base">Loading map...</p>
        </div>
      </div>
    );
  ...
  const handleMouseMove = (e) => {
    ...
    if (stateId && vaccinationData[stateId]) {
      ...
      setTooltip({ visible: true, x, y, stateId, stateName: stateData.name, data: stateData });
    }
  };
  ...
  const handleStateClick = (stateId) => {
    ...
    if (isMobile) {
      setTooltip({ visible: true, x: window.innerWidth / 2, y: window.innerHeight / 2, stateId, stateName: stateData.name, data: stateData });
      setIsMobilePopupOpen(true);
    } else {
      setTooltip({ visible: true, x: 0, y: 0, stateId, stateName: stateData.name, data: stateData });
      setIsMobilePopupOpen(false);
    }
  };
  ...
  if (updatedSvg.includes("<svg")) {
    updatedSvg = updatedSvg
      .replace(/\swidth="[^"]*"/, "")
      .replace(/\sheight="[^"]*"/, "")
      .replace("<svg", `<svg class="w-full h-auto max-w-full" preserveAspectRatio="xMidYMid meet"`);
  }
    ...
    return (
      <div className="w-full">
        <ResponsiveContainer width="100%" height={100}>
          <PieChart>
            <Pie data={pieData} cx="50%" cy="50%" innerRadius={20} outerRadius={40} paddingAngle={2} dataKey="value">
        ...
        <div className="text-center text-[10px] text-gray-400 mt-1">Total: {new Intl.NumberFormat().format(total)}</div>
      </div>
    );
  };
  ...
  return (
    <div ref={mapRef} className="relative w-full flex justify-center">
      ...
      {selectedState && tooltip.data && !isMobile && (
        <div className="fixed bg-slate-800/95 backdrop-blur-md shadow-2xl rounded-lg p-5 w-72 z-50 border border-white/20 animate-fade-in"
         style={{
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3 text-[10px] sm:text-xs md:text-sm mt-2">
            {[
              ...
              <div key={label} className={`bg-${color}-500/20 p-2 rounded-lg border border-${color}-400/30`}>
                ...
                <div className="text-white font-bold text-xs sm:text-base md:text-lg mt-1">{new Intl.NumberFormat().format(value)}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-slate-900/95 backdrop-blur-md rounded-3xl p-5 w-11/12 max-w-sm z-50 animate-fade-in border border-white/20">
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={() => setIsMobilePopupOpen(false)} className="text-gray-400 hover:text-white text-lg">
                ×
              </button>
            </div>
            <StatePieChart data={tooltip.data} />
